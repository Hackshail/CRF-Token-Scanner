...
app.secret_key = os.getenv(
    "DASHBOARD_SECRET_KEY", "change-this-in-production-dashboard"
)
...
def dashboard_stats():
    ...
    try:
        ...
        health_data = (
            health_response.json() if health_response else {"status": "unknown"}
        )
        ...
        stats = {
            "health": health_data,
            "active_scans": len(
                [s for s in scans_data.get("scans", []) if s["status"] == "running"]
            ),
            "total_scans": scans_data.get("total", 0),
            "completed_scans": len(
                [s for s in scans_data.get("scans", []) if s["status"] == "completed"]
            ),
            "failed_scans": len(
                [s for s in scans_data.get("scans", []) if s["status"] == "failed"]
            ),
            "metrics": metrics,
...
def scan_detail(scan_id):
    ...
    return render_template(
        "scan_detail.html", scan_id=scan_id, user=session.get("user")
    )
...
if __name__ == "__main__":
    ...
    app.run(debug=True, host="0.0.0.0", port=DASHBOARD_PORT, threaded=True)

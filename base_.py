...
class CSRFScanner:
    ...
    def scan(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Scan completed in {scan_duration:.2f}s. Found {len(self.results)} forms")
            return self.results
    ...
    def _process_form(self, form, page_url: str):
        ...
        logger.debug(f"Found form: {method} {action} - Token: {'Yes' if csrf_token else 'No'}")
...
if __name__ == '__main__':
    try:
        ...
        for result in results:
            print(f"  {result['method']} {result['action']} - Status: {result['status']}")

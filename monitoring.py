...
class MetricsCollector:
    ...
    def observe_histogram(self, name: str, value: float, labels: Dict[str, str] = None):
        ...
        if name not in self.metrics:
            self.metrics[name] = {"type": "histogram", "observations": [], "labels": {}}
        ...
        if len(self.metrics[name]["observations"]) > 1000:
            self.metrics[name]["observations"] = self.metrics[name]["observations"][
                -1000:
            ]
...
class AlertManager:
    ...
    def _check_scan_failure_rate(self, metrics: MetricsCollector) -> bool:
        ...
        failed_scans = metrics.metrics.get("csrf_scans_failed_total", {}).get(
            "value", 0
        )
    ...
    def _check_rate_limits(self, metrics: MetricsCollector) -> bool:
        ...
        rate_limit_hits = metrics.metrics.get("rate_limit_exceeded_total", {}).get(
            "value", 0
        )
...
class HealthChecker:
    ...
    def perform_health_check(self) -> Dict:
        ...
        # Overall status
        failed_checks = [
            k for k, v in health["checks"].items() if not v.get("healthy", False)
        ]
        if failed_checks:
    ...
    def _check_cpu(self) -> Dict:
        ...
        return {"healthy": cpu_percent < 95, "usage_percent": cpu_percent}
        ...
        try:
            ...
            if hasattr(result, "__len__") and len(result) > 0:
                metrics_collector.increment_counter(
                    "csrf_vulnerabilities_found_total",
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_collector.observe_histogram(
                    f"api_request_duration_seconds",
                    duration,

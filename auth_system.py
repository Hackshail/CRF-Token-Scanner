...
class AuthConfig:
    ...
    def __init__(self):
        self.jwt_secret = os.getenv('JWT_SECRET_KEY', 'change-this-in-production-very-long-random-string')
...
class AuthManager:
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[User]:
        ...
        if bcrypt.checkpw(password.encode(), stored_hash.encode()):
            self._audit_log('login_success', username, request.remote_addr if request else 'unknown')
            return user

        self._audit_log('login_failed', username, request.remote_addr if request else 'unknown')
        return None
...
def create_user(username: str, password: str, role: UserRole, email: str) -> bool:
    ...
    hashed_password = bcrypt.hashpw(password.encode(), bcrypt.gensalt(auth_manager.config.bcrypt_rounds))

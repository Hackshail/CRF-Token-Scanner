...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'csrf_scan_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
Examples:
  ...
  python main.py --api
    ...
    parser.add_argument('--max-urls', type=int, default=100, help='Maximum URLs to crawl')
    
    # Reporting options
    parser.add_argument('-r', '--report', choices=['json', 'csv', 'html', 'all'], 
                       default='all', help='Report format(s) to generate')
    parser.add_argument('-f', '--filename', type=str, default='csrf_report',
                       help='Base filename for reports')
    parser.add_argument('-o', '--output-dir', type=str, default='reports',
    ...
    parser.add_argument('--no-external', action='store_true', help='Don\'t follow external links')
    ...
    try:
        ...
        # Scanning mode
            ...
            if results is not None:
                # Generate reports
                if generate_reports(results, args.report, args.filename, args.output_dir):
